---
title: "vLLM vs Hugging Face TEI cho embedding và rerank: nên chọn cái nào?"
slug: vllm-vs-hugging-face-tei-embedding-rerank
authors: [manhpt]
tags: [embedding, rerank, llm, technical, vietnamese]
date: 2026-04-12
description: "So sánh thực dụng giữa vLLM và Hugging Face Text Embeddings Inference cho hai bài toán embedding và rerank, bám theo tài liệu chính thức thay vì benchmark cảm tính."
---

Nếu đang dựng một stack RAG hoặc semantic search, câu hỏi thực tế không còn là “có chạy được embedding không”, mà là: **nên dùng gì để serve embedding và rerank cho production**. Hai cái tên thường được đưa lên bàn cân là **vLLM** và **Hugging Face Text Embeddings Inference (TEI)**.

Điểm thú vị là cả hai đều đã hỗ trợ embedding và rerank. Nhưng nếu đọc kỹ docs chính thức, có thể thấy chúng đang tối ưu cho **hai bài toán vận hành khác nhau**.
...
- **Chọn TEI** nếu bạn muốn một service chuyên cho **embedding và rerank**, boot nhanh, footprint gọn, API đơn giản, vận hành khá thẳng.
- **Chọn vLLM** nếu bạn đã tiêu chuẩn hóa hạ tầng quanh **vLLM/OpenAI-compatible server**, hoặc muốn gom nhiều workload inference vào cùng một runtime.
- Nếu mục tiêu là **throughput tốt nhất cho embedding/rerank thuần túy**, docs hiện tại của vLLM thậm chí còn nói khá thẳng rằng phần pooling models của họ **mới chủ yếu là để tiện dụng**, và **chưa đảm bảo nhanh hơn** việc dùng Hugging Face Transformers hoặc Sentence Transformers trực tiếp.

Chỉ riêng chi tiết đó đã làm cán cân nghiêng khá mạnh về phía TEI cho use case embedding/rerank chuyên biệt.
...
Đây là một tín hiệu kỹ thuật đáng chú ý. Nó cho thấy embedding/rerank trong vLLM hiện tại là **một capability hữu ích**, nhưng chưa chắc là **đường tối ưu nhất** nếu đó là workload chính của bạn.
...
Nói cách khác, **vLLM thắng ở sự hợp nhất**, không phải vì embedding là trọng tâm duy nhất của nó.
...
Nếu bạn cần cross-encoder scoring và đã có cụm vLLM sẵn rồi, vLLM hoàn toàn có thể là lựa chọn hợp lý. Nhưng nếu bạn đang hỏi “cái nào tự nhiên hơn cho rerank”, tôi nghiêng về **TEI**.
...
Còn TEI thì gần như sinh ra cho đúng workload này.
...
- **TEI** là lựa chọn mặc định rất mạnh cho embedding và rerank chuyên biệt.
- **vLLM** phù hợp hơn khi bạn ưu tiên một runtime hợp nhất cho nhiều loại inference task.
...
Với tôi, đó là khác biệt đủ lớn để ảnh hưởng quyết định kiến trúc.
