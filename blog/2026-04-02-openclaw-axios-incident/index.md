---
title: "Nếu bạn bật auto-update cho OpenClaw, hãy kiểm tra máy ngay sau sự cố Axios"
description: "Phân tích rủi ro thực tế cho các hệ thống OpenClaw tự host có auto-update sau sự cố axios@1.14.1 và axios@0.30.4 bị phát tán mã độc qua chuỗi cung ứng npm."
authors: [manhpt]
tags: [openclaw, security, npm, supply-chain, javascript, incident, vietnamese]
date: 2026-04-02
---

Nếu bạn đang chạy **OpenClaw** theo kiểu tự host và có bật **auto-update** hoặc một cơ chế cập nhật định kỳ tương tự, tôi nghĩ bạn nên **kiểm tra máy ngay lập tức**.

Lý do không phải vì đã có bằng chứng công khai kết luận **official `openclaw` package** chắc chắn phát tán mã độc. Tôi chưa thấy bằng chứng như vậy, và bài này không khẳng định điều đó. Vấn đề nằm ở chỗ khác: trong cửa sổ sự cố **Axios compromise** cuối tháng 3/2026, các hệ thống tự động cập nhật dependency có thể đã kéo phải một package độc hại trong chuỗi phụ thuộc, và với kiểu agent stack chạy nhiều secret, nhiều integration, hậu quả có thể rất lớn.
...
Điểm quan trọng nhất ở đây là: nếu một máy đã cài đúng các version đó trong cửa sổ bị ảnh hưởng, bạn nên **giả định máy đã bị compromise** cho đến khi chứng minh được điều ngược lại.

## Vì sao người dùng OpenClaw nên quan tâm ngay?

Tôi muốn tách rất rõ ba mệnh đề sau:
...
Tuy nhiên, vẫn có một lý do rất thực tế để các operator OpenClaw phải điều tra ngay:

- Trong source tree OpenClaw hiện tại, package chính phụ thuộc `@line/bot-sdk`
- Trong source `@line/bot-sdk`, `axios` xuất hiện dưới `optionalDependencies` với dải version `^1.7.4`
- Trong một cây cài đặt OpenClaw mà tôi kiểm tra, `axios` hiện diện ở version **`1.14.0`**

Ba điểm trên **chỉ đủ để chứng minh dependency path là có thật**: `openclaw -> @line/bot-sdk -> axios`. Nó **không chứng minh** máy của bạn đã từng kéo `axios@1.14.1` trong quá khứ. Và đây là chỗ nhiều người dễ kết luận quá tay.

Nói ngắn gọn:

> OpenClaw có một đường phụ thuộc hợp lý dẫn tới Axios. Nếu hệ thống của bạn auto-update đúng vào cửa sổ sự cố, bạn có rủi ro thực tế. Nhưng trạng thái filesystem hiện tại có thể không đủ để chứng minh lịch sử phơi nhiễm trong cửa sổ đó.
...
Đó là lý do tôi nghiêng về hướng **assume breach** với các host đã cài package độc hại trong cửa sổ bị ảnh hưởng, thay vì cố tự trấn an bằng việc hiện tại `node_modules` trông có vẻ sạch.
...
## Một caveat rất quan trọng: trạng thái hiện tại có thể đánh lừa bạn
...
Nên **current state không nhất thiết phản ánh historical exposure**.
...
2. Rà lockfile, install log, CI log để xác định có từng kéo `axios@1.14.1`, `axios@0.30.4`, hoặc `plain-crypto-js@4.2.1` hay không.
3. Kiểm tra IOC filesystem và network nêu trên.
4. Rotate toàn bộ secrets có thể đã từng hiện diện trên máy:
   - npm tokens
...
5. Audit tất cả CI runners, builder, cron host, dev workstation nào có thể đã rebuild hoặc reinstall dependency trong cửa sổ sự cố.
6. Nếu có dấu hiệu rõ ràng, **rebuild máy từ known-good state**, không cố "lau sạch tại chỗ".
...
- [LINE Bot SDK package manifest (`axios` under `optionalDependencies`)](https://raw.githubusercontent.com/line/line-bot-sdk-nodejs/master/package.json)
